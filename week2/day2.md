...
```
fastapi              # 用於建立高效能的 Python Web API 框架，支援自動產生 OpenAPI 文件
uvicorn              # ASGI 伺服器，常與 FastAPI 搭配使用以提供非同步支援
openai               # OpenAI 官方 API 套件，用於與 OpenAI 服務（如 GPT、DALL·E）互動
python-dotenv        # 用於載入 .env 檔案中的環境變數，方便管理敏感資訊與設定
python-multipart     # 處理 multipart/form-data（如檔案上傳）的套件，FastAPI 需要
boto3                # AWS SDK for Python，與 Amazon S3、DynamoDB 等 AWS 服務互動
pypdf                # 處理 PDF 檔案的套件，可用於讀取、分割、合併 PDF
mangum               # 讓 ASGI 應用（如 FastAPI）可在 AWS Lambda 上執行的轉接器
```
...
這個檔案是 FastAPI 應用的主程式，負責提供 AI 聊天 API 服務，並支援對話記憶（可選擇本地或 S3 儲存）。其主要功能如下：

1.API 伺服器啟動：建立 FastAPI 應用，設定 CORS，讓前端（如 React）可以安全存取 API。

2.環境變數載入：用 dotenv 載入設定（如 OpenAI API 金鑰、CORS 來源、S3 參數等）。

3.OpenAI 聊天模型串接：初始化 OpenAI API 客戶端，將用戶訊息與系統提示送給 GPT-4o-mini 模型，取得 AI 回覆。

4.對話記憶管理：根據設定，對話紀錄可儲存在本地檔案或 AWS S3。每個 session_id 對應一份對話紀錄，支援多輪對話。
...
/chat：主要聊天端點，接收訊息、回傳 AI 回覆，並更新對話紀錄。

/conversation/{session_id}：查詢指定 session 的完整對話紀錄。

6.錯誤處理：API 端點有例外處理，發生錯誤時會回傳 HTTP 500。

總結：這個檔案讓你可以用 REST API 方式，與 AI 進行多輪對話，並且支援對話記憶的儲存與查詢。
...
這個檔案的作用是讓 FastAPI 應用（server.py）可以在 AWS Lambda 上執行。它透過 Mangum 這個套件，把 ASGI 應用（FastAPI 屬於 ASGI）包裝成 Lambda handler，讓 AWS API Gateway 可以呼叫你的 FastAPI API。簡單來說，這是 AWS Lambda 部署 FastAPI 的入口檔案。
...
(deploy.py是一個自動化部署腳本，用來建立 AWS Lambda 的部署套件（zip 檔）。它的主要流程如下：

1.清理舊的部署資料：刪除舊的 lambda-package 資料夾與 lambda-deployment.zip 檔案，確保每次都是全新打包。

2.建立新資料夾：建立 lambda-package 資料夾，作為所有 Lambda 需要檔案的暫存區。

3.安裝相依套件：利用 Docker 執行 AWS Lambda 官方 Python 3.12 映像檔，並在相容的環境下安裝 requirements.txt 內的所有套件到 lambda-package，確保相依性與 Lambda 執行環境一致。

4.複製應用程式檔案：將 server.py、lambda_handler.py、context.py、resources.py 這些應用程式檔案複製到 lambda-package。

5.複製資料資料夾：如果有 data 資料夾，也一併複製進去。

6.壓縮成 zip：將 lambda-package 內所有內容壓縮成 lambda-deployment.zip，這個 zip 檔就是 Lambda 上傳用的部署包。7.顯示檔案大小：最後顯示壓縮檔的大小，方便檢查是否超過 Lambda 限制。

這個腳本讓你可以一鍵產生 AWS Lambda 可用的部署包，確保相依性與檔案都正確無誤。)
